...
<h2 className="font-semibold w-max text-4xl mx-auto">Choose Your Plan</h2>
<p className="mx-auto w-max pt-9">Let's choose the package that is best for you and explore it happily and cheerfully.</p>

    <div className="grid mx-auto w-[88%] mt-16 mb-16 gap-y-7  md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3">
        ...
        <div className="pl-[3.5rem] mt-[43px] space-y-4">
          {subscription.benefits.map((benefit, index) => (
            <div className="flex items-center" key={index}>
              ...
              <span className="text-sm mx-4 text-gray-700">{benefit}</span>
            </div>
          ))}
        </div>
        <div className="flex-grow"></div>

        <h4 className="mx-auto mt-2 text-4xl font-semibold text-gray-800">
          {subscription.price}
          <span className="text-base font-normal">
            {subscription.currency}
          </span>{subscription.type && (
          <span className="text-3xl text-gray-700 font-normal"> / {subscription.type}</span>
        )}
        </h4>
